...
class TestTextNode(unittest.TestCase):
    ...
    def test_to_html(self):
        ...
        self.assertEqual(
            expected_text, textnode.text_node_to_html_node(text_node).to_html()
        )
        self.assertEqual(
            expected_bold, textnode.text_node_to_html_node(bold_node).to_html()
        )
        self.assertEqual(
            expected_italic, textnode.text_node_to_html_node(italic_node).to_html()
        )
        self.assertEqual(
            expected_code, textnode.text_node_to_html_node(code_node).to_html()
        )
        self.assertEqual(
            expected_link, textnode.text_node_to_html_node(link_node).to_html()
        )
        self.assertEqual(
            expected_img, textnode.text_node_to_html_node(img_node).to_html()
        )
...
* with items"""
    ...
    def test_block_to_block_type(self):
        ...
        self.assertEqual(
            textnode.block_to_block_type(blocks[0]), textnode.BlockType.PARAGRAPH
        )
        self.assertEqual(
            textnode.block_to_block_type(blocks[1]), textnode.BlockType.HEADING
        )
        self.assertEqual(
            textnode.block_to_block_type(blocks[2]), textnode.BlockType.CODE
        )
        self.assertEqual(
            textnode.block_to_block_type(blocks[3]), textnode.BlockType.QUOTE
        )
        self.assertEqual(
            textnode.block_to_block_type(blocks[4]), textnode.BlockType.ULIST
        )
        self.assertEqual(
            textnode.block_to_block_type(blocks[5]), textnode.BlockType.OLIST
        )
